...
0.5.0에서 완료:
- `roost/state.py` — `team_state.json`으로 프로젝트 큐, 이벤트 로그, 컨텍스트 축적
- `roost/preset.py` — zip 기반 프리셋 내보내기/가져오기
- `roost/backend/hermes.py` — Hermes Agent 연동 LLM 이벤트 분류 (선택)
- `roost/security.py` — L0~L3 프로젝트별 보안 레벨
- `roost/backend/script.py` — 컨텍스트 기반 우선순위 보정
- 상태바 알바 상태 아이콘 (🟢⚪🔴)
