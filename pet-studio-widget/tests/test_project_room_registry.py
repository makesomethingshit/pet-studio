...
class ProjectRoomSceneTests(unittest.TestCase):
    ...
    def test_project_hub_owns_team_room_view(self) -> None:
        ...
        self.assertIn('notebook.add(daily_tab, text="Daily")', text)
        self.assertIn('notebook.add(advanced_tab, text="Advanced")', text)
        self.assertIn('advanced_notebook.add(team_tab, text="Team Room")', text)
        self.assertIn('advanced_notebook.add(endpoints_tab, text="Endpoints")', text)
        self.assertIn("def _build_team_room_tab(", text)
        self.assertIn("Pet Studio Workroom", text)
        self.assertNotIn('text="Active model"', text)
        self.assertNotIn('text="Team mode"', text)
        self.assertNotIn("header_bot", text)
        self.assertIn("initial_id = current_id if current_id in project_map", text)
        self.assertIn("tree.selection_set(initial_id)", text)
        ...
        self.assertIn("masked_auth_status", text)
        self.assertIn("Connection settings saved.", text)
        ...
        self.assertIn("test_model_profile(profile)", text)
        self.assertNotIn("Codex profile saved", text)
        self.assertIn('text="Export"', text)
        self.assertIn('text="Import"', text)
    ...
    def test_local_auth_file_is_ignored_and_wizard_is_clean(self) -> None:
        ...
        self.assertIn("CODEX_AUTH_TOKEN", wizard)
        self.assertIn("http://127.0.0.1:8787/v1", wizard)
...
class ProjectRoomRegistryTests(unittest.TestCase):
    ...
    def test_widget_cli_manages_model_profiles_without_gui(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(tested_payload["diagnostics"]["liveProbe"]["source"], "script")
